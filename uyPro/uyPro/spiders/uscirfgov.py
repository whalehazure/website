...
from .utils import start_spider, update_ch_urls, process_emails
from .webmod import get_map


class UscirfgovSpider(scrapy.Spider):
    name = "uscirfgov"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.uscirf.gov/event-type/hearings'
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://www.uscirf.gov/news-room/press-releases-statements',
            'https://www.uscirf.gov/event-type/hearings',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div[@id='main']//div[@class='content']//div[@class='view-content row']/div//a/@href"
                               ).getall()
        for link in links:
            if not link:
                continue
        ...
        next_url = response.xpath("//ul/li[@class='pager__item--next']/a/@href").get('')
    ...
    def article(self, response):
        ...
        if tweet_func:
            ...
            if item:
                tweet_pdf_urls = response.xpath("//div[@id='main']//div[@class='content']//div["
                                                "@class='full-content']/node()[self::p or self::ul]//a/@href").getall()

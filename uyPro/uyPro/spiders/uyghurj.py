...
class UyghurjSpider(scrapy.Spider):
    name = "uyghurj"
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div[@class='info']/h4[@class='title']/a/@href").getall()
        for link in links:
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            # if self.redis_conn.sismember('uyghurj_done_urls', link_hash) and self.inc:
        ...
        next_url = response.xpath("//a[@class='next page-numbers']/@href|//h3[@class='headline1']/span/a/@href").get()
        ...
        if tweet_func:
            ...
            if item:
                ...
                if not item.get('tweet_content') or item.get('tweet_content_tslt'):
                    ...
                    # self.redis_conn.sadd('uyghurj_done_urls', link_hash)
                yield item

...
class GknbgovkgSpider(scrapy.Spider):
    name = "gknbgovkg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://gknb.gov.kg/kg/%d0%b6%d0%b0%d0%bd%d1%8b%d0%bb%d1%8b%d0%ba%d1%82%d0%b0%d1%80-%d0%b0%d1%80%d1%85%d0%b8%d0%b2%d0%b8/'
            # inputdata = {}
            # tweeturl = ''
            self.taskid = taskid
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://gknb.gov.kg/kg/%d0%b6%d0%b0%d0%bd%d1%8b%d0%bb%d1%8b%d0%ba%d1%82%d0%b0%d1%80-%d0%b0%d1%80%d1%85%d0%b8%d0%b2%d0%b8/'
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//article[@id]/a/@href").getall()
        for link in links:
        ...
        next_url = response.xpath("//nav/a[@class='next page-numbers']/@href").get()
        ...
        if tweet_func:
            ...
            if item:
                ...
                item['tweet_lang'] = detect_language(article_content) if article_content else detect_language(
                    article_title)
                item['tweet_img'] = response.meta.get('tweet_img', [])
                link_hash = hashlib.sha1(link.encode()).hexdigest()

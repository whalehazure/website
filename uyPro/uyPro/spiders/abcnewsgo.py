...
class AbcnewsgoSpider(scrapy.Spider):
    name = "abcnewsgo"
    ...
    def start_requests(self):
        ...
        try:
            ...
            elif method == 'getchannel' and '/search?' in churl:
                url = (f'https://abcnews.go.com/meta/api/search?limit=10&sort=date&type=&section=&totalrecords=true'
                       f'&offset=0&q={churl.split("searchtext=")[1].split("&")[0]}')
                yield scrapy.Request(url=url, callback=self.parse_trd, meta={'ch_url': churl, 'offset': 10})
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            ...
            'https://abcnews.go.com/search?searchtext=china&userab=abc_web_player-460*variant_b_abc_dmp-1901,'
            'otv_web_player-461*variant_a_otv_control-1902&sort=date',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//section[@class='ContentRoll__Item']/div/h2/a/@href").getall()
        for link in links:
            if not link:
                continue
    ...
    def parse_trd(self, response):
        ...
        itms = response.json().get('item', [])
        for itm in itms:
            link = itm.get('link', '')
            if not link or 'abcnews.go.com/images/' in link:
                continue
            link = response.urljoin(link)
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
        ...
        if if_new:
            next_url = (f'https://abcnews.go.com/meta/api/search?limit=10&sort=date&type=&section=&totalrecords=true'
                        f'&offset={offset}&q={ch_url.split("searchtext=")[1].split("&")[0]}')
            yield response.follow(url=next_url, callback=self.parse_trd, meta={'ch_url': ch_url, 'offset': offset + 10})

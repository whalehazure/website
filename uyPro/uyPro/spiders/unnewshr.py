...
class UnnewshrSpider(scrapy.Spider):
    name = "unnewshr"
    ...
    def __init__(self, name=None):
        ...
        self.bid = ''
        self.inc = ''
        self.proname = "unnewshr"

    def start_requests(self):
        # homepage = 'https://news.un.org/zh/news/topic/human-rights'
        # link = 'https://news.un.org/zh/story/2023/11/1123742'
        # churl = 'https://news.un.org/zh/news/topic/human-rights'
        homepage = ''
        try:
    ...
    def parse(self, response, **kwargs):
        links = [
            'https://news.un.org/zh/news/topic/human-rights',
        ]
        for link in links:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath(
            "//div[@class='view-content']/div[@class='views-row']//h2[@class='node__title']/a/@href").getall()
        for link in links:
            link = response.urljoin(link)
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            # if self.redis_conn.sismember('unnewshr_done_urls', link_hash) and self.inc:
        ...
        next_url = response.xpath("//li[contains(@class,'page-item')]/a[@rel='next']/@href").get()
    ...
    def article(self, response):
        ...
        article_title = response.xpath(
            "string(//div[@class='views-row']//span[contains(@class,'field--name-title')])").get('').strip()
        if article_title:
            item['tweet_title'] = article_title
            item['tweet_title_tslt'] = ''
            ps = response.xpath("//main[@id='content']//div[@class='field-content']//node()[self::p or self::h4]")
            ...
            item['tweet_createtime'] = parse_date(response.xpath("string(//meta[@property='article:published_time"
                                                                 "']/@content)").get('').strip())
            item['tweet_img_url'] = response.xpath("//meta[@property='og:image']/@content|//main[@id='content']//img["
                                                   "@title]/@src").getall()
            item['tweet_table'] = ''
            if '<table' in response.xpath("//main[@id='content']//div[@class='field-content']").get(''):
                html_content = response.xpath("//main[@id='content']//div[@class='field-content']").get()

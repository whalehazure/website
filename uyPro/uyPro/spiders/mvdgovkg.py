...
class MvdgovkgSpider(scrapy.Spider):
    name = "mvdgovkg"
    ...
    def parse_fir(self, response):
        ...
        link = 'https://mvd.gov.kg/api/kg/news/?kids=false'
    ...
    def parse_sec(self, response):
        ...
        ch_url = response.meta['ch_url']
        _json = response.json()
        _dates = _json.get('results', []) if isinstance(_json, dict) else []
        for _date in _dates:
            if not _date:
                continue
            _id = _date.get('id', '')
            link = f'https://mvd.gov.kg/news/{_id}'

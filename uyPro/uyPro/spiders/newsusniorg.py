...
class NewsusniorgSpider(scrapy.Spider):
    name = "newsusniorg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://news.usni.org/category/fleet-tracker'
            url = 'https://r.jina.ai/' + churl
            tweeturl1 = 'https://r.jina.ai/' + tweeturl
            # inputdata = {}
            self.taskid = taskid
            ...
            if homepage:
                ...
                yield scrapy.Request(url=url, callback=self.parse_sec, headers=headers, meta={'ch_url': churl})
            else:
                yield scrapy.Request(url=tweeturl1, callback=self.article, headers=headers, meta={'ch_url': churl, 'link': tweeturl})
        except TypeError as e:
            logging.info(f'exit:{e}')

    def parse(self, response, **kwargs):
        chlinks = [
            'https://news.usni.org/latest',
            'https://news.usni.org/popular',
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div/article/div[@class='post56__text']/node()[self::h2 or self::h3]/a/@href").getall()
        for link in links:
            ...
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ...
                if_new = True
                theurl = 'https://r.jina.ai/' + link
                headers = {'X-Return-Format': 'html'}
                yield response.follow(theurl, callback=self.article, headers=headers, meta={'ch_url': ch_url, 'link': link})
        next_url = response.xpath("//div[@class='pagination56']/a[@class='next page-numbers']/@href").get()
        if next_url and if_new:
            next_url = 'https://r.jina.ai/' + next_url
            headers = {'X-Return-Format': 'html'}

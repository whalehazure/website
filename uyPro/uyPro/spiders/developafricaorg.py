...
class DevelopafricaorgSpider(scrapy.Spider):
    name = "developafricaorg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.developafrica.org/recent-news'
            # inputdata = {}
            # tweeturl = 'https://www.developafrica.org/taxonomy/term/1/feed'
            self.taskid = taskid
    ...
    def parse(self, response, **kwargs):
        initial_url = response.xpath("//div[@id='block-gavias-kunco-system-main']//a[@class='feed-icon']/@href").get()
        if initial_url:
            yield scrapy.Request(url=initial_url, callback=self.parse_rss,
                                 meta={'ch_url': response.meta['ch_url'], 'original_response': response})
        else:
            yield from self.parse_sec(response)

    def parse_rss(self, response):
        items = response.xpath("//channel/item")
        for item in items:
            link = item.xpath("./link/text()").get()
            timecontent = item.xpath("./pubDate/text()").get()
            if link:
                self.results[link] = timecontent
        original_response = response.meta.get('original_response')
        yield from self.parse_sec(original_response)

    def parse_sec(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        links = response.xpath("//ul[@class='links inline']/li[@class='node-readmore']/a/@href").getall()
        for link in links:
            if not link:
                continue
            link = response.urljoin(link)
            timecontent = self.results.get(link, '')
            ...
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ...
                if_new = True
                # print(link, timecontent)
                yield response.follow(link, callback=self.article,
                                      meta={'ch_url': ch_url, 'link': link, 'timecontent': timecontent})

        next_url = response.xpath("//ul[@class='js-pager__items']/li/a[@rel='next']/@href").get()
    ...
    def article(self, response):
        ...
        timecontent = response.meta['timecontent']
        ...
        if tweet_func:
            ...
            if item:
                item['tweet_createtime'] = parse_date(timecontent) if timecontent else item.get('tweet_createtime', '')
                link_hash = hashlib.sha1(link.encode()).hexdigest()

...
class MinghuiorgSpider(scrapy.Spider):
    name = "minghuiorg"
    ...
    def parse(self, response, **kwargs):
        ...
        next_url = response.xpath("//div[@class='fp_navigation']/a[text()='按日浏览']/@href").get('')
        if next_url:
            yield response.follow(next_url, callback=self.parse_list, meta={'ch_url': ch_url})

    def parse_list(self, response):
        ch_url = response.meta['ch_url']
        self.alllinks = response.xpath("//table[@id='archive_daily_table']//td//table//td/a[@class='txt2cal'][1]/@href"
                                       ).getall()
        next_url = self.alllinks.pop()
        if next_url:
            next_url += 'todaynews.html'
            yield response.follow(next_url, callback=self.parse_sec, meta={'ch_url': ch_url})

    def parse_sec(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        links = response.xpath("//div[@class='featureitem']/div[@class='info']/a[1]/@href|//div["
                               "@id]/div/table//td/a/@href").getall()
        for link in links:
            if not link:
                continue

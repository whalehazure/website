...
class NationthailandSpider(scrapy.Spider):
    name = "nationthailand"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.nationthailand.com/category/news'
            # inputdata = {}
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://www.nationthailand.com/category/the-opinion',
            'https://www.nationthailand.com/category/news',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        if next_data:
            try:
                ...
                next_url = data.get('props', {}).get('pageProps', {}).get('nextArticles')
            except json.JSONDecodeError:
                logging.warning('Failed to parse __NEXT_DATA__ JSON')
        for link in links:
            if not link:
                continue
        ...
        if next_url and if_new:
            next_url = urljoin('https://api.nationthailand.com', next_url)
    ...
    def parse_trd(self, response):
        ...
        if next_data:
            try:
                ...
                logging.warning('Failed to parse JSON from API response')
        for link in links:
            if not link:
                continue
            link = urljoin('https://www.nationthailand.com', link)

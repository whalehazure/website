...
from .utils import start_spider, update_ch_urls, increment_url
from .webmod import get_map


class VoachineseSpider(scrapy.Spider):
    name = "voachinese"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.voachinese.com/s?k=%E6%96%B0%E7%96%86&tab=all&pi=1&r=any&pp=10'
            # inputdata = {}
            self.taskid = taskid
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            ...
            'https://www.voachinese.com/s?k=%E6%96%B0%E7%96%86&tab=all&pi=1&r=any&pp=10',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        next_url = response.xpath("//div[@class='media-block-wrap']/p[@class='buttons btn--load-more']/a/@href").get()
        if next_url:
            links = response.xpath("//div[contains(@class,'pull-left')]//div[@class='row']/ul/li/div/div/a/@href").getall()
        else:
            links = response.xpath("//li/div/div/a/@href").getall()
        for link in links:
        ...
        next_url_s = response.xpath("//ul[@class='pagination__list']/li/a[contains(@class,'btn--next')]/@href").get()

        if next_url and if_new:
            yield response.follow(url=next_url, callback=self.parse_sec, meta={'ch_url': ch_url})
        elif next_url_s and if_new:
            yield response.follow(url=next_url_s, callback=self.parse_sec, meta={'ch_url': ch_url})
        elif if_new:
            next_url = increment_url(response.url)
            yield response.follow(url=next_url, callback=self.parse_sec, meta={'ch_url': ch_url})

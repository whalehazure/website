...
class UygurhaberSpider(scrapy.Spider):
    name = "uygurhaber"
    allowed_domains = ["uygurhaber.com"]
    ...
    def __init__(self, name=None):
        ...
        self.bid = ''
        self.inc = ''
        self.proname = "uygurhaber"

    def start_requests(self):
        # homepage = 'https://www.uygurhaber.com/'
        # link = 'https://www.uygurhaber.com/kurulusunun-90-yilinda-dogu-turkistan-cumhuriyeti-ve-gunumuzde-dogu' \
        #        '-turkistan-paneli-duzenlenecek/ '
        # churl = 'https://www.uygurhaber.com/'
        homepage = ''
        try:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//section[@id='main']/div[@id='loop-container']/div//h2/a/@href").getall()
        for link in links:
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            # if self.redis_conn.sismember('uygurhaber_done_urls', link_hash) and self.inc:
    ...
    def article(self, response):
        ...
        item['tweet_lang'] = 'tr'
        article_title = response.xpath("string(//article/div[@class='post-header']/h1[@class='post-title'])").get()
        if article_title:
            ...
            article_content = response.xpath("string(//article/div[@class='post-content'])").get().replace('\xa0',
                                                                                                           ' ').strip()
            if article_content:
            ...
            item['tweet_video'] = ''
            item['tweet_author'] = response.xpath("string(//div[@class='post-author']/div/div[@class='author'])").get()
            item['tweet_createtime'] = parse_date(response.xpath("string(//meta[@property='article:published_time"
                                                                 "']/@content)").get('').strip())
            item['tweet_img_url'] = response.xpath("//article/div[@class='featured-image']/img/@src|//div["
                                                   "@class='post-content']/div["
                                                   "@class='wp-block-image']/figure/img/@src").getall()
            item['tweet_table'] = ''
            if '<table' in response.xpath("//article/div[@class='post-content']").get(''):
                html_content = response.xpath("//article/div[@class='post-content']").get()
            ...
            if not item.get('tweet_content') or item.get('tweet_content_tslt'):
                ...
                # self.redis_conn.sadd('uygurhaber_done_urls', link_hash)
            yield item

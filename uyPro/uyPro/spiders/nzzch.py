...
class NzzchSpider(scrapy.Spider):
    name = "nzzch"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.nzz.ch/neueste-artikel'
            # inputdata = {}
    ...
    def parse_fir(self, response):
        link = 'https://enrico.nzz.ch/v2/newest-articles?product=nzz&limit=16&offset=0'
        yield response.follow(link, callback=self.parse_sec, meta={'ch_url': link, 'offset': 0})

    def parse_sec(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        offset = response.meta['offset']
        metadatas = response.json().get('data', [])
        for metadata in metadatas:
            link = metadata.get('metadata', {''}).get('url', '')
            if not link:
                continue
        ...
        offset += 16
        next_url = f'https://enrico.nzz.ch/v2/newest-articles?product=nzz&limit=16&offset={offset}'
        if next_url and if_new:
            yield response.follow(url=next_url, callback=self.parse_sec, meta={'ch_url': ch_url, 'offset': offset})

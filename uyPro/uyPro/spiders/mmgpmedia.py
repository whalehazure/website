...
class MmgpmediaSpider(scrapy.Spider):
    name = "mmgpmedia"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'http://www.mmgpmedia.com/web/RD/index.html'
            # inputdata = {}
    ...
    def parse(self, response, **kwargs):
        ...
        for link in chlinks:
            yield response.follow(link, callback=self.parse_fir, meta={'ch_url': link})

    def parse_fir(self, response, **kwargs):
        ch_url = response.meta['ch_url']
        pageNum = 1
        pageSize = 10
        siteId = 752495108259188736
        channel_match = re.search(r'var\s+channelId\s*=\s*["\'](\d+)["\']', response.text)
        if channel_match:
            channelId = channel_match.group(1)
        else:
            logging.warning('Failed to extract channelId from response')
            return
        link = f'http://api.offshoremedia.net/cms/front/list/latest?pageNum={pageNum}&pageSize={pageSize}' \
               f'&siteId={siteId}&channelId={channelId}'
        yield response.follow(link, callback=self.parse_sec,
                              meta={'ch_url': ch_url, 'pageNum': pageNum, 'channelId': channelId})
    ...
    def parse_sec(self, response):
        ...
        lists = response.json().get('info', {}).get('list', [])
        for list in lists:
            link = list.get('contentStaticPage')
            try:
                contentPublishTime = str(list.get('contentPublishTime'))[:10]
            except:
                contentPublishTime = None
            if not link:
                continue
        ...
        if if_new:
            pageNum += 1
            next_url = f'http://api.offshoremedia.net/cms/front/list/latest?pageNum={pageNum}&pageSize={pageSize}' \
                       f'&siteId={siteId}&channelId={channelId}'
            yield response.follow(url=next_url, callback=self.parse_sec, meta={'ch_url': ch_url, 'pageNum': pageNum,
                                                                               'channelId': channelId})

    def article(self, response):
        item = UyproItem()
        link = response.meta['link']
        contentPublishTime = response.meta['contentPublishTime']
        ...
        if tweet_func:
            item = tweet_func(response, item, contentPublishTime)
            if item:
                link_hash = hashlib.sha1(link.encode()).hexdigest()

...
from .utils import start_spider, update_ch_urls, detect_language, translatetext_bing
from .webmod import get_map


class UyghurstudyorgSpider(scrapy.Spider):
    name = "uyghurstudyorg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://ug.uyghurstudy.org/category/xeverler/'
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://ug.uyghurstudy.org/category/xeverler/',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div[@class='row']//div[@class='blg-pst-wrp']/div//h4/a/@href").getall()
        for link in links:
            if not link:
                continue
        ...
        next_url = response.xpath("//ul[@class='pagination']/li[contains(@class,'page-item next')]/a/@href").get('')
        if if_new and next_url:
    ...
    def article(self, response):
        ...
        if tweet_func:
            ...
            if item:
                ...
                tweet_title_tslt = item.get('tweet_title_tslt', '')
                tweet_content_tslt = item.get('tweet_content_tslt', '')
                lang = detect_language(article_content) if article_content else detect_language(article_title)
                item['tweet_lang'] = lang
                if detect_language(tweet_title_tslt) != 'zh':
                    item['tweet_title_tslt'] = translatetext_bing(article_title)
                if not tweet_content_tslt:
                    item['tweet_content_tslt'] = translatetext_bing(article_content)
                tweet_pdf_urls = response.xpath("//div[@class='row']//div//div[contains(@class,"
                                                "'detail-desc')]/p/a/@href").getall()
                tweet_pdf_urls = [url for url in tweet_pdf_urls if 'pdf' in url]

...
class NewsltncomtwSpider(scrapy.Spider):
    name = "newsltncomtw"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://news.ltn.com.tw/list/breakingnews/politics'
            # inputdata = {}
            self.taskid = taskid
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//article[@class='boxTitle']//a/@href|//ul[@class='list']/li/a/@href").getall()
        for link in links:
        ...
        turl = response.xpath('//script').re_first(r"url:\s*'([^']*)'")
        ttype = response.xpath('//script').re_first(r"let type = '(.+?)';")
        turl = f'{turl}{ttype}/'
        if if_new and turl:
            pagenum = 2
            url = f'{turl}{pagenum}'
            yield scrapy.Request(url, callback=self.parse_trd, meta={'ch_url': ch_url, 'pagenum': pagenum, 'turl': turl})

    def parse_trd(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        pagenum = response.meta['pagenum']
        turl = response.meta['turl']
        expression = parse("$..url")

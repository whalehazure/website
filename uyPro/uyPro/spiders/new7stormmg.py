...
class New7stormmgSpider(scrapy.Spider):
    name = "new7stormmg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://new7.storm.mg/category/k320014'
            # inputdata = {}
            self.taskid = taskid
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://new7.storm.mg/category/k248039',
            'https://new7.storm.mg/category/k320014',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div/div[@class='card']/div[@class='card_inner']/a/@href").getall()
        for link in links:
        ...
        kid = response.xpath("//div[@id='category_bottom']/category-component/@kid").get('')
        # csrftoken = response.xpath("//div[@id='category_bottom']/category-component/@csrf-token").get('')
        if if_new:
            url = f'https://new7.storm.mg/api/category/{kid}'
            yield scrapy.Request(url, callback=self.parse_trd, meta={'ch_url': ch_url, 'kid': kid})
    ...
    def parse_trd(self, response):
        ...
        expression1 = parse("$.articles.first_section.[*]")
        expression2 = parse("$.articles.other_section.[*]")
        sections = expression1.find(rejson) + expression2.find(rejson)
        for section in sections:
            nid = section.value.get('nid')
            link = f'https://new7.storm.mg/article/{nid}'
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ch_urls_json = self.redis_conn.hget(f'{self.proname}_hash_done_urls', link_hash)
                ch_urls = json.loads(ch_urls_json) if ch_urls_json else []
                ...
                if tweet_func:
                    item = tweet_func(section.value, item)
                    if item:
                        link_hash = hashlib.sha1(link.encode()).hexdigest()
                        ...
                        yield item
        if if_new:
            lastnid = sections[-1].value.get('nid')
            lastpub = sections[-1].value.get('article_date_publish')
            url = f'https://new7.storm.mg/api/category/{kid}/{lastpub}/{lastnid}'
            yield scrapy.Request(url, callback=self.parse_trd, meta={'ch_url': ch_url, 'kid': kid})

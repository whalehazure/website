import hashlib
import json
import logging

import scrapy
from scrapy.selector import Selector
from uyPro.items import UyproItem
from uyPro.settings import redis_conn
from .utils import start_spider, update_ch_urls, detect_language, process_emails
from .webmod import get_map


class EnactafricaorgSpider(scrapy.Spider):
    name = "enactafricaorg"
    redis_conn = redis_conn
    custom_settings = {
        'ITEM_PIPELINES': {'uyPro.pipelines.CustomFilesPipeline': 300, },
        'AUTOTHROTTLE_ENABLED': True,
        'AUTOTHROTTLE_MAX_DELAY': 10,
        'USER_AGENT': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/'
                      '537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36',
        # 'LOG_ENABLED': True
    }

    def __init__(self, name=None):
        super().__init__(name)
        self.taskid = ''
        self.bid = ''
        self.inc = ''
        self.proname = name

    def start_requests(self):
        homepage = ''
        try:
            taskid, method, churl, tweeturl, dltype, inputdata, inputfilename, recent_files_append = start_spider()
            # taskid, method, churl, tweeturl, dltype, inputdata, inputfilename, recent_files_append = (
            #     '45ca39c6ebcfa6449f672481fc4a084b_1705450920', 'getchannel', '', '', 'full', '', '1_1_1_1', '')
            # churl = 'https://enactafrica.org/research/continental-reports'
            # inputdata = {}
            self.taskid = taskid
            self.bid = inputfilename.split('_')[3]
            self.crawler.stats.set_value('inputdata', inputdata)
            self.crawler.stats.set_value('inputfilename', inputfilename)
            self.crawler.stats.set_value('recent_files_append', recent_files_append)
            self.inc = False if dltype == 'full' else True
            if homepage:
                yield scrapy.Request(url=homepage, callback=self.parse)
            elif method == 'getchannel':
                yield scrapy.Request(url=churl, callback=self.parse_sec, meta={'ch_url': churl})
            else:
                yield scrapy.Request(url=tweeturl, callback=self.article, meta={'ch_url': churl, 'link': tweeturl})
        except TypeError as e:
            logging.info(f'exit:{e}')

    def parse(self, response, **kwargs):
        chlinks = [
            'https://enactafrica.org/enact-observer',
            'https://enactafrica.org/research/policy-briefs',
            'https://enactafrica.org/research/research-papers',
            'https://enactafrica.org/research/continental-reports',
            'https://enactafrica.org/research/interpol-reports',
            'https://enactafrica.org/research/ocwar-t',
            'https://enactafrica.org/events/',
        ]
        for link in chlinks:
            yield response.follow(link, callback=self.parse_sec, meta={'ch_url': link})

    def parse_sec(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        divs = response.xpath("//div[@id='children_append']/div/a")
        for div in divs:
            link = div.xpath("./@href").get('')
            title = div.xpath("string(./div/h6)").get('')
            if not link:
                continue
            link = response.urljoin(link)
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ch_urls_json = self.redis_conn.hget(f'{self.proname}_hash_done_urls', link_hash)
                ch_urls = json.loads(ch_urls_json) if ch_urls_json else []
                if ch_url in ch_urls:
                    logging.info(f'{link} : repetition')
                else:
                    item = UyproItem()
                    item['ch_url'] = ch_url
                    item['tweet_id'] = link
                    item['taskid'] = self.taskid
                    item['bid'] = self.bid
                    ch_urls.append(ch_url)
                    self.redis_conn.hset(f'{self.proname}_hash_done_urls', link_hash, json.dumps(ch_urls))
                    if_new = True
                    yield item
            else:
                if_new = True
                # print(link)
                lang = detect_language(title)
                if lang == 'fr':
                    continue
                yield response.follow(link, callback=self.article, meta={'ch_url': ch_url, 'link': link})
        slug = response.xpath("//input[@id='page_slug']/@value").get('')
        ptype = response.xpath("//input[@id='page_type_id']/@value").get('')
        impact = response.xpath("//input[@id='page_impact']/@value").get('')
        page = 1
        next_url = f'https://enactafrica.org/children?type={ptype}&slug={slug}&page={page}&impact={impact}'
        if if_new and ptype:
            yield scrapy.Request(next_url, callback=self.parse_trd,
                                 meta={'ch_url': ch_url, 'page': page, 'ptype': ptype, 'slug': slug, 'impact': impact})

    def parse_trd(self, response):
        if_new = False
        ch_url = response.meta['ch_url']
        page = response.meta['page'] + 1
        ptype = response.meta['ptype']
        slug = response.meta['slug']
        impact = response.meta['impact']
        divs = response.xpath("//div/a")
        for div in divs:
            link = div.xpath("./@href").get('')
            title = div.xpath("string(./div/h6)").get('')
            if not link:
                continue
            link = response.urljoin(link)
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ch_urls_json = self.redis_conn.hget(f'{self.proname}_hash_done_urls', link_hash)
                ch_urls = json.loads(ch_urls_json) if ch_urls_json else []
                if ch_url in ch_urls:
                    logging.info(f'{link} : repetition')
                else:
                    item = UyproItem()
                    item['ch_url'] = ch_url
                    item['tweet_id'] = link
                    item['taskid'] = self.taskid
                    item['bid'] = self.bid
                    ch_urls.append(ch_url)
                    self.redis_conn.hset(f'{self.proname}_hash_done_urls', link_hash, json.dumps(ch_urls))
                    if_new = True
                    yield item
            else:
                if_new = True
                # print(link)
                lang = detect_language(title)
                if lang == 'fr':
                    continue
                yield response.follow(link, callback=self.article, meta={'ch_url': ch_url, 'link': link})
        next_url = f'https://enactafrica.org/children?type={ptype}&slug={slug}&page={page}&impact={impact}'
        if if_new and ptype:
            yield scrapy.Request(next_url, callback=self.parse_trd,
                                 meta={'ch_url': ch_url, 'page': page, 'ptype': ptype, 'slug': slug, 'impact': impact})

    def article(self, response):
        item = UyproItem()
        link = response.meta['link']
        item['ch_url'] = response.meta['ch_url']
        item['tweet_url'] = response.url
        item['tweet_id'] = link
        item['taskid'] = self.taskid
        item['bid'] = self.bid
        tweet_func = get_map(response.url)
        if tweet_func:
            html_content = process_emails(response.body)
            new_response = Selector(text=html_content)
            item = tweet_func(new_response, item)
            if item:
                article_content = item.get('tweet_content', '')
                article_title = item.get('tweet_title', '')
                item['tweet_lang'] = detect_language(article_content) if article_content else detect_language(
                    article_title)
                link_hash = hashlib.sha1(link.encode()).hexdigest()
                if not item.get('tweet_content') or item.get('tweet_content_tslt') or item['tweet_lang'] == 'zh':
                    update_ch_urls(self.redis_conn, self.proname, link_hash, item['ch_url'])
                yield item

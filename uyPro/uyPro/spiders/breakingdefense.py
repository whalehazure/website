...
class BreakingdefenseSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://breakingdefense.com/tag/indo-pacific/'
            # inputdata = {}
            self.taskid = taskid
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//h1[@class='postTitle main-featured__title']/a/@href|//div["
                               "@id='mainContent']/div//h3[@class='postTitle']/a/@href").getall()
        for link in links:
        ...
        if if_new:
            pagenum = 1
            url = "https://breakingdefense.com/wp-admin/admin-ajax.php"
            data = {
                "action": "bm_ajax_load_more",
                "page": str(pagenum),
                "query": "{\"tag\":\"indo-pacific\",\"category_name\":\"\",\"tag_id\":15179,\"paged\":0,\"post__not_in\":[],\"tag__in\":[],\"post_type\":\"\"}",
                "security": "e63fac64c4"
            }
            yield scrapy.FormRequest(url, formdata=data, callback=self.parse_trd, meta={'ch_url': ch_url, 'pagenum': pagenum})
    ...
    def parse_trd(self, response):
        ...
        links = selector.xpath("//div[@class='post-loop']//h3[@class='postTitle']/a/@href").getall()
        for link in links:
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ch_urls_json = self.redis_conn.hget(f'{self.proname}_hash_done_urls', link_hash)
        ...
        if if_new:
            ...
            yield scrapy.FormRequest(url, formdata=data, callback=self.parse_trd,
                                     meta={'ch_url': ch_url, 'pagenum': pagenum})

...
class MfagovirSpider(scrapy.Spider):
    name = "mfagovir"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://mfa.gov.ir/portal/newsagencyshow/701'
            # inputdata = {}
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://mfa.gov.ir/portal/newsagencyshow/683',
            'https://mfa.gov.ir/portal/newsagencyshow/3179',
            'https://mfa.gov.ir/portal/newsagencyshow/701'
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath("//div[@class='row']//div[@class='news-arch-item']//div["
                               "@class='news-item-title']/h2/a[@class='hTitle']/@href").getall()
        for link in links:
            if not link:
                continue
        ...
        href_value = response.xpath("//nav/ul/li[@class='page-item active']/following-sibling::li[1]/a/@href")
        matches = href_value.re(r'\"(uit\$.*?)\",.*?\"(\d+\?startrow=\d+)')
        if matches and if_new:
            url = response.urljoin(matches[1])
            __EVENTTARGET = matches[0]
            ...
            formdata = {
                ...
                "yspForgeryToken": "",
                "uit%24ctl00%24ycw86146%24ctl00%24txtq": ""
            }
            yield scrapy.FormRequest(url=url, method='post', formdata=formdata, callback=self.parse_sec,
                                     meta={'ch_url': ch_url})

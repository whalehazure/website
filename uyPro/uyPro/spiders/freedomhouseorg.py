...
class FreedomhouseorgSpider(scrapy.Spider):
    name = "freedomhouseorg"
    ...
    def __init__(self, name=None):
        ...
        self.bid = ''
        self.inc = ''
        self.proname = "freedomhouseorg"

    def start_requests(self):
        # homepage = 'https://freedomhouse.org/'
        # link = 'https://freedomhouse.org/article/freedom-house-welcomes-four-new-trustees'
        # churl = 'https://freedomhouse.org/'
        homepage = ''
        try:
            ...
            # churl = 'https://freedomhouse.org/news-perspectives'
            # inputdata = {}
            self.taskid = taskid
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            ...
            'https://freedomhouse.org/news-perspectives',
            'https://freedomhouse.org/perspectives',
            'https://freedomhouse.org/news-perspectives/press-releases-statements/press-releases',
        ]
    ...
    def parse_sec(self, response):
        ...
        links = response.xpath(
            "//div[contains(@class,'node--view-mode-card')]/a[2]/@href|//div[contains(@class,"
            "'paragraph--type--image-plus-text')]/a/@href|//div[@class='one-up-cta-card-wrapper']/div[contains(@class,"
            "'field--type-link')]/a/@href").getall()
        for link in links:
            link = response.urljoin(link)
            link_hash = hashlib.sha1(link.encode()).hexdigest()
            # if self.redis_conn.sismember('freedomhouseorg_done_urls', link_hash) and self.inc:
            ...
            if (self.redis_conn.sismember(f'{self.proname}_done_urls', link_hash) or self.redis_conn.hexists(
                ...
                if 'washingtonpost.com' in link:
                    ...
                    yield response.follow(link, callback=self.article, headers=headers, meta={'ch_url': ch_url, 'link': link})
                else:
                    yield response.follow(link, callback=self.article, meta={'ch_url': ch_url, 'link': link})
        next_url = response.xpath("//li[@class='pager__item pager__item--next']/a/@href").get()
    ...
    def article(self, response):
        ...
        if tweet_func:
            ...
            print(response.url, 'no tweet_func')

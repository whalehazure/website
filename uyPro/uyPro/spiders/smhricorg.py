...
from .utils import start_spider, update_ch_urls, translatetext, translatetext_bing
from .webmod import get_map


class SmhricorgSpider(scrapy.Spider):
    name = "smhricorg"
    ...
    def start_requests(self):
        ...
        try:
            ...
            # churl = 'https://www.smhric.org/news.htm'
            # inputdata = {}
    ...
    def parse(self, response, **kwargs):
        chlinks = [
            'https://www.smhric.org/news.htm',
            'https://www.smhric.org/smw_chinese.htm',
        ]
        for link in chlinks:
    ...
    def parse_sec(self, response):
        ...
        aas = response.xpath("//table[@align='center']/tr/td[2]//a|//td[@class='c1']/table/tr/td/blockquote/p/font/a")
        for aa in aas[:50]:
            link = aa.xpath("./@href").get()
            if not link or link == 'index.htm':
                continue
            ...
            if self.redis_conn.hexists(f'{self.proname}_hash_done_urls', link_hash) and self.inc:
                ...
                # print(link)
                if any(ext in link for ext in ('.jpg', '.png', '.jpeg', '.gif', '.JPG')):
                    continue
                if '.pdf' in link:
                    item = UyproItem()
                    item['ch_url'] = ch_url
                    item['tweet_url'] = link
                    item['tweet_id'] = link
                    lang = 'zh' if 'smw_chinese.htm' in ch_url else 'en'
                    item['tweet_lang'] = lang
                    item['taskid'] = self.taskid
                    item['bid'] = self.bid
                    tweet_title = aa.xpath("string(.)").get('').strip().replace('\n', '').replace('\t', '')
                    item['tweet_title'] = tweet_title
                    item['tweet_title_tslt'] = ''
                    if lang == 'en':
                        item['tweet_title_tslt'] = translatetext_bing(tweet_title) or translatetext(tweet_title)
                    ...
                    tweet_title = aa.xpath("string(.)").get('').strip().replace('\n', '').replace('\t', '')
                    yield response.follow(link, callback=self.article, meta={
                        'ch_url': ch_url, 'link': link, 'tweet_title': tweet_title})
    ...
    def article(self, response):
        ...
        item['tweet_id'] = link
        lang = 'zh' if 'smw_chinese.htm' in item['ch_url'] else 'en'
        item['tweet_lang'] = lang
        item['taskid'] = self.taskid
        item['bid'] = self.bid
        tweet_func = get_map(response.url)
        if tweet_func:
            item = tweet_func(response, item, lang == 'en', response.meta.get('tweet_title', ''))
            if item:
                link_hash = hashlib.sha1(link.encode()).hexdigest()
